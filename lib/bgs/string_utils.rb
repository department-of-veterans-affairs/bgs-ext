module BGS
  module StringUtils
    ...
    def self.camelize_but_use_ID_for_id(string, mode = DEFAULT_CAMELIZE_MODE)
      snake_case_converter(string, '', proc { |w| to_ID(w) || CAMELIZE_MODE_TO_PROC.send(mode).(w) }) do |w|
    ...
    def self.snake_case_converter(
      ...
      [
        ...
        *words[1..-1].map(&converter)
      ].join(join_with)
    end

    private_class_method def self.to_ID(string)
      /id/i === string && 'ID'
    end
    ...
    UPCASE_FIRST_CHARACTER = proc { |string| "#{string[0]&.upcase}#{string[1..-1]}" }
    private_constant :UPCASE_FIRST_CHARACTER

    DOWNCASE_FIRST_CHARACTER = proc { |string| "#{string[0]&.downcase}#{string[1..-1]}" }
